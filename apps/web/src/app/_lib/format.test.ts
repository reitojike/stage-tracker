...
describe("occurrenceTimeRangeLabel", () => {
  ...
  it("shows the 終了時刻未定 label when endsAt is unset (`specs/005-event-occurrence-lifecycle/spec.md`「公演日程」: nullable end time is a valid state)", () => {
    expect(
      occurrenceTimeRangeLabel(
  ...
  it("shows the actual end date instead of （翌日） when the end is 2+ Tokyo calendar days after the start (codex review 指摘: starts_at <= ends_at has no 24h cap - `specs/005-event-occurrence-lifecycle/spec.md`「開場 / 開演 / 終演」)", () => {
