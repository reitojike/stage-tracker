...
/**
 * Ported from the historical M8 comparison's `occurrenceTimeRangeLabel` (codex review
 * 指摘: `/catalog` の選択日一覧が
 * 開始時刻しか表示せず、終了時刻の有無・翌日終了を区別できなかった修正）。
 *
 * 終演時刻は不明な場合があり、未設定を正当な状態として扱う
 * （`specs/005-event-occurrence-lifecycle/spec.md`「公演日程」）ので、既知の終了時刻が無いことを「終了時刻
 ...
 * 「（翌日）」を付けて区別する。
 *
 * `starts_at <= ends_at` はDB level のproduct invariant（`specs/005-event-occurrence-lifecycle/spec.md`「開場 /
