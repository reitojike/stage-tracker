...
export function KabukiManualShadowRun() {
  ...
  async function start() {
    ...
    try {
      const response = await fetch("/api/official-import/shadow", {
        ...
        body: JSON.stringify({ sourceId: KABUKI_SOURCE_ID }),
      });
  ...
  return (
    <section
      ...
      aria-labelledby="kabuki-run-heading"
    >
      <div className="flex flex-col gap-2xs">
        <SectionHeading id="kabuki-run-heading">
          歌舞伎の手動取得
        </SectionHeading>
        <p className="text-body-sm text-muted-foreground">
          公式情報を一度だけ確認し、解析できた内容を確認待ち候補にします。自動承認・反映はしません。実行後は数分待ってこの画面を更新し、候補と保留ページを確認してください。
        </p>
      </div>
      <div>
        <Button
          ...
          onClick={start}
        >
          {status === "starting" ? "取得を開始中…" : "歌舞伎を手動取得"}
        </Button>
      </div>
      {status === "started" ? (
