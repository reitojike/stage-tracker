...
export interface CatalogViewProps {
  ...
  /** `null` = 月ランディング（未選択）。`specs/011-catalog-classification-filter/spec.md`。 */
 ...
 * (`specs/005-event-occurrence-lifecycle/spec.md` 「イベントカタログ一覧」,
 * `specs/011-catalog-classification-filter/spec.md`). A Client Component because filter
 * selection is held client-local and persisted to `localStorage`
 * (specs/011-catalog-classification-filter/spec.md "Filter persistence": "browser-local persistenceで十分" - no
 ...
 * own `CatalogView.tsx` (the current contract) never renders such a flat landing list
