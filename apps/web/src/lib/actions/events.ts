...
/**
 * Event/Occurrence lifecycle and ownership semantics follow Spec 005. This
 * module owns the exact write-layer behavior for the
 ...
 * （current RPC/RLS/schema contract）、ここでの分類は DB が返したエラーを共通の
...
export const updateEventRangeAction = authActionClient
  ...
  .action(async ({ parsedInput, ctx }) => {
    ...
    // （current runtime contract「期間編集はSheet内フォームで…」）
