 ...
 *
 * `docs/architecture/authentication.md` の browser/server separation における server client に
 * 対応する。anon key のみを使い、`next/headers` の `cookies()` から
 * セッションを読み書きする。
 *
 * Passkey Auth API は supabase-js でデフォルト有効になっている。通常の server client を
 * 共有し、cookie wiring をこの factory に集約する。
 *
 ...
 * 担保している」ことに依存する（`docs/architecture/authentication.md`）。
 ...
 *
 * `docs/architecture/authentication.md` の browser/server separation における cookieless server
