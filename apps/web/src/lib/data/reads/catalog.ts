 ...
 * (`specs/005-event-occurrence-lifecycle/spec.md` の `listEventCatalogInRange`)。
 *
 * `specs/005-event-occurrence-lifecycle/spec.md`「Catalog の日程参照要件」の2つの要件——(a) 期間内に公演回が
 ...
 * 収まる（`specs/005-event-occurrence-lifecycle/spec.md`）ため、「期間内に
 ...
 * `using (true)` の shared catalog（`specs/005-event-occurrence-lifecycle/spec.md`）
 ...
 * values を返す（specs/011-catalog-classification-filter/spec.md「Filter option universe」: 表示中の月には
 ...
 * canonical identity は genre へ hard-bind されないが（specs/011-catalog-classification-filter/spec.md
 ...
 * （specs/011-catalog-classification-filter/spec.md「Filter option universe」）。
 *
 ...
 * hard-bind されない（specs/011-catalog-classification-filter/spec.md「Group」）ため、この読み方は genre に
 ...
 * specs/011-catalog-classification-filter/spec.md「Group」の 0..N）が `supabase/config.toml` の `api.max_rows`
 ...
 * グループとも数十件規模、specs/011-catalog-classification-filter/spec.md「Catalog classification / venue
 ...
 * は canonical master を持たない生 text（specs/011-catalog-classification-filter/spec.md「Venue」）なので、
