...
export function toActionErrorShape(error: Error): ActionErrorShape<string> {
 ...
 *
 * `this module and its tests` の「各 action は権限判定を一切行わない。
 ...
 * RLS の代替ではない（同 の `requireAuthenticatedUserId` と同じ
 * 位置づけ）。
 */
