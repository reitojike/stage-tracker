...
describe("AppBar", () => {
  it("renders the bell and My Page avatar by default", () => {
    ...
    expect(
      screen.getByRole("button", { name: "お知らせ" }),
    ).toBeInTheDocument();
  ...
  it("hides both affordances when showActions=false (unauthenticated surface)", () => {
    render(
      <AppBar showActions={false} myPageHref="/mypage" myPageInitial="A" />,
    );

    expect(
      screen.queryByRole("button", { name: /お知らせ/ }),
  ...
  it("hasUnreadNotifications only changes the accessible label/marker, it is never derived internally", () => {
    render(
      <AppBar myPageHref="/mypage" myPageInitial="A" hasUnreadNotifications />,
    );

    expect(
      screen.getByRole("button", { name: "お知らせ（未読あり）" }),
    ).toBeInTheDocument();
  });
});
