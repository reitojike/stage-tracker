...
describe('PrimaryNav', () => {
  it('renders exactly the 4 fixed items (decisions.md P2)', () => {
    ...
    expect(screen.getByRole('link', { name: /イベント/ })).toHaveAttribute('href', '/catalog');
    expect(screen.getByRole('link', { name: /チケット/ })).toHaveAttribute('href', '/tickets');
  ...
  it('treats nested routes as active (e.g. /catalog/events/1 highlights イベント)', () => {
    ...
    expect(screen.getByRole('link', { name: /イベント/ })).toHaveAttribute('aria-current', 'page');
  });
