...
// onto one of the redefined Badge variants (outline/subtle/done/deadline/
// terminal - 'done' added by #186). Labels themselves are untouched by #138
// and are covered here only to pin the existing pairing each status renders
// with.

void test('ticketDisplayStatusBadgeVariant maps secured to done (Issue #196: a user-completed action, matching Badge.tsx\'s own #186 "チケット確保済み" example)', () => {
  assert.equal(ticketDisplayStatusBadgeVariant('secured'), 'done');
});
