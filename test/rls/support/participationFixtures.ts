...
import type { Database } from '../../../apps/web/src/lib/data/database.types.ts';
 ...
 * each call site re-deriving the same check. The generated RPC type cannot
 * express PostgREST's all-null composite representation, so the runtime id
 * check is intentional at this raw DB-test boundary.
 */
export async function declineInvitation(
  ...
  return { data: data !== null && typeof data.id === 'string' ? data : null, error };
}
