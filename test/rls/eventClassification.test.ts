...
type QueryClient = TestActor['client'];

async function listCatalogGenres(client: QueryClient) {
...
async function listCatalogGroupOptions(client: QueryClient, genreId: string) {
  const { data, error } = await client
    .from('event_groups')
    .select('groups(id, key, display_name), events!inner(genre_id)')
    .eq('events.genre_id', genreId);
  if (error !== null) return { ok: false as const, error };
  const byId = new Map<string, { id: string; key: string; displayName: string }>();
  for (const row of data) {
    byId.set(row.groups.id, {
      id: row.groups.id,
      key: row.groups.key,
      displayName: row.groups.display_name,
    });
  }
  return {
    ok: true as const,
    data: [...byId.values()].sort((a, b) => a.displayName.localeCompare(b.displayName, 'ja')),
  };
}

async function listCatalogVenueOptions(client: QueryClient, genreId: string) {
  const { data, error } = await client
    .from('events')
    .select('venue')
    .eq('genre_id', genreId)
  ...
  return {
    ok: true as const,
    data: [...new Set(data.flatMap((row) => (row.venue === null ? [] : [row.venue])))].sort(
      (a, b) => a.localeCompare(b, 'ja'),
    ),
  };
}

async function getEventClassificationsByIds(client: QueryClient, eventIds: readonly string[]) {
  if (eventIds.length === 0) return { ok: true as const, data: [] };
  const [eventsResult, groupsResult, genresResult] = await Promise.all([
    client.from('events').select('id, genre_id').in('id', eventIds),
    client
      .from('event_groups')
      .select('event_id, groups(id, key, display_name)')
      .in('event_id', eventIds),
    listCatalogGenres(client),
  ]);
  if (eventsResult.error !== null) return { ok: false as const, error: eventsResult.error };
  if (groupsResult.error !== null) return { ok: false as const, error: groupsResult.error };
  if (!genresResult.ok) return genresResult;

  const genresById = new Map(genresResult.data.map((genre) => [genre.id, genre] as const));
  const groupsByEvent = new Map<string, { id: string; key: string; displayName: string }[]>();
  for (const row of groupsResult.data) {
    const groups = groupsByEvent.get(row.event_id) ?? [];
    groups.push({
      id: row.groups.id,
      key: row.groups.key,
      displayName: row.groups.display_name,
    });
    groupsByEvent.set(row.event_id, groups);
  }
  ...
  return {
    ...
    data: eventsResult.data.map((event) => ({
      eventId: event.id,
      genre: event.genre_id === null ? null : (genresById.get(event.genre_id) ?? null),
      groups: (groupsByEvent.get(event.id) ?? []).sort((a, b) =>
...
// raw read projections above. Runtime mapping/read behavior belongs to
// apps/web unit tests; this file proves the real local DB/RLS boundary.
//
