...
try {
  ...
  // reporting success.
  //
  // Sequential, not concurrent: these actors can share event/occurrence
  // rows, so running the deletes together can have one actor remove a parent
  // while another is still deleting its children. The helper exists for
  // exactly that reason and already aggregates per-actor failures.
  let cleanupError;
  try {
