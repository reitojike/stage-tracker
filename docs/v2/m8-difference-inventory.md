...
1. **意図した差分** — 下記「意図した差分（実コードで確認済み）」節の項目に
   一致するもの
2. **v2 の不具合** — 下記「意図した差分」節に無い、oracle からも外れた差分
3. **legacy の不具合を v2 が正したもの** — 「意図した差分」節の項目のうち、
   legacy 側の既知バグを修正する形になっているもの（各項目に明記する）

対象は **観測可能な product behavior の差分**に限る。実装技術の違い
（CSS Modules → Tailwind 等）はここに含めない。

**「本 inventory の項目に一致する」の範囲**: 上記 1 の「本 inventory」は
下記「意図した差分（実コードで確認済み）」節だけを指す。下記「既知の v2
不具合」節は、並走比較を待たず**この時点で既に分類 2 と判明している**項目を
先出しで記録したものであり、そこに載っているというだけの理由で分類 1 に
昇格しない（それでは既知の不具合を意図した差分として扱ってしまう）。
...
代わりに oracle（`docs/v2/oracle-routes-ui.md`）または `AGENTS.md` の
該当箇所を citation する。
