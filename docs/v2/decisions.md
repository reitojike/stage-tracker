...
| F3  | Windows 上の `pnpm install` が `packages/domain` の peer dependency を持つ devDependency（vitest / vite / eslint 等）に対して壊れたリンクを生成する事象が観測された。lockfile 自体は正しく、clean reinstall や `--force` でも再現した。pnpm の Windows junction リンクの不具合と見られる。CI（ubuntu）で `packages/domain` が正しく install されるかを必ず確認すること |
