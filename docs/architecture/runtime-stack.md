...
3. スキーマ変更を伴う PR の場合、`main` への push を検知した
   ...
   保証しません**（historical rationale: Issue #387）。新しい build が直ちに参照する migration を分離した場合、
   ...
   - 変えるなら（DB が出す値の変更・既存 reader が読む列や制約の変更等）、
     ...
     （historical rationale: Issue #393）。
...
--include-all --skip-vault --yes` を実行する。`SUPABASE_DB_URL` は
   ...
   historical rationale (Issue #393). deploy の起動・抑止には一切関与せず、
...
- 歌舞伎sourceは、個別詳細ページの厳密な日程解析だけが失敗した場合、そのページを
  ...
  同じ開演回の既存終演時刻が今回の公式情報から消えた場合も、その公演の候補を作らず
  別の定型理由コードで報告し、既存の時刻は自動削除しません。
